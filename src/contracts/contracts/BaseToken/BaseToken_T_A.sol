...
contract BaseToken_T_A is IERC20, ERC20Ownable, TokenHelper {
    ...
    constructor(
        ...
        uint256 _initial,
        address _router,
        uint8 _tax_reflection,
        uint8 _tax_wallet,
        uint8 _tax_burn,
        uint8 _tax_liquidity,
        uint8 _tax_buyback
    ) TokenHelper("BaseToken_T_A") {
        _name = _name;
        _symbol = _symbol;
        _decimals = _decimals;
        _tTotal = _initial;
        ...
        buyBackUpperLimit = _initial.mul(1).div(10000);

        _rOwned[_msgSender()] = _rTotal;

        feeWallet = payable(_msgSender());

        numTokensSellToAddToLiquidity = _initial.mul(1).div(1000);
        ...
        _liquidityFee = _tax_liquidity;
        _burnFee = _tax_burn;
        _buybackFee = _tax_buyback;
